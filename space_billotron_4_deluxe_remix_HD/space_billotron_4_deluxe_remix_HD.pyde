...
def setup():
    ...
    feu = loadImage("feu-d-artifice.jpg")
        ...
        if player == 1: #si c'est au tour du joueur 1
            ...
            if shoot == 1:
                if sqrt((x-xDeux)**2+(y-yDeux)**2) < 4*rayonBalle:
                    xTarget=(x+xDeux)/2
                    yTarget=(y+yDeux)/2                    
                    xTarget+=random(-1*(rayonBalle),(rayonBalle))
                    yTarget+=random(-1*(rayonBalle),(rayonBalle)) 
                                       
                elif sqrt((xTrois-xDeux)**2+(yTrois-yDeux)**2)<=sqrt((x-xTrois)**2+(y-yTrois)**2):
                    xTarget=xDeux
                    yTarget=yDeux                    
                    xTarget+=random(-1.5*(rayonBalle),1.5*(rayonBalle))
                    yTarget+=random(-1.5*(rayonBalle),1.5*(rayonBalle))  
...
def move(): 
    ...
    distanceUNtoTROIS=sqrt((x-xTrois)**2+(y-yTrois)**2)        
        ...
        #calcul des rebonds entre les boules 
                    
    if distanceUNtoDEUX<=2*rayonBalle :          #si la boule un touche la boule deux              
        
        dx=(x-xDeux)                            #on détermine la différence de postion des boules en x et en y 
        dy=(y-yDeux)
        
        sUn=speed                        # on transfère les variables des vitesse des boules dans des variables de calcul
        sDeux=speedYUn
        sTrois=speedXDeux
        sQuatre=speedYDeux
 
        correction = (2*rayonBalle - distanceUNtoDEUX)*0.5  #on calcul la distance sur laquelle on va devoir déplacer les boules pour les séparer
        
        x=x+(dx/distanceUNtoDEUX)*correction              #on applique cette correction aux positions des boules (trigonométrie)  
        y=y+(dy/distanceUNtoDEUX)*correction
        xDeux=xDeux-(dx/distanceUNtoDEUX)*correction
        yDeux=yDeux-(dy/distanceUNtoDEUX)*correction
        
        dx=(x-xDeux)    #on recalcule la différence de postion des boules en x et en y car ont les a déplacées
        dy=(y-yDeux)
        
        nx=(dx)/(2*rayonBalle)  #on détermine les coordonnés en x et en y du vecteur n de norme 1 et normale au plan de contact entre les 2 boules
        ny=(dy)/(2*rayonBalle)
        gx=-ny        # idem pour g qui est tangent au plan de contact
        gy=nx
        
        vAn=nx*sUn+ny*sDeux     #on exprime la vitesse des boules en fonction de n et g
        vAg=gx*sUn+gy*sDeux
        vBn=nx*sTrois+ny*sQuatre
        vBg=gx*sTrois+gy*sQuatre
        
        speed= nx*vBn+gx*vAg       #on réassigne la vitesse après rebond de chaque boule
